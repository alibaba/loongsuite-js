...
| `gen_ai.usage.total_tokens` | response | `gen_ai.usage.total_tokens`（优先上游，缺失则算 input+output）| [SHOULD] |
...
> **total_tokens 取值（0.1.0-beta.9+）**：LLM span 的 `gen_ai.usage.total_tokens` **优先采用上游报告值**；仅当上游未提供、或值为 0 而 input/output 非零（不可用）时，才回退计算 `input + output`。因此若上游能拿到 provider 返回的真实 total（尤其含 cache 的场景，如 Anthropic，input+output 会偏小），应直接在 `llm.response` 上携带 `gen_ai.usage.total_tokens`；拿不到则省略，由转换器兜底。
...
- `gen_ai.usage.total_tokens`：**优先累加上游报告值**——仅当 turn 内每条带 token 的 `llm.response` 都携带了 `total_tokens`（且 > 0）时，取它们之和；只要有一条缺失或为 0，整个 AGENT 回退为 `Σinput + Σoutput`（避免上游值与计算值混用）。
...
### 3.6 自定义字段透传（`ConvertOptions.passthroughKeys`，0.1.0-beta.9+）

除上述固定映射外，**转换器的调用方**（如 pilot）可通过 `passthroughKeys` 白名单，把 event log 里的任意自定义字段原样（字段名即 span 属性名，不改名）写到 span 上。这是消费侧的可选能力，**不改变**上游写事件的约束。

```ts
convertEventLogToTrace(records, {
  handler,
  passthroughKeys: ["deployment.env", "gen_ai.request.temperature"],
});
```

- **粒度**：turn 级字段（该 turn 任一事件携带）广播到该 turn 的**所有** span；LLM/TOOL 额外从各自的源 record（request/response、call/result）读取，同名时 per-record 覆盖 turn 级。
- **fill-only**：仅当 span 尚未携带该属性时才写入，**永不覆盖**转换器已产出的属性（token、model、公共属性、TTFT 等）。要覆盖这些需直接构建 invocation 并用 `invocation.attributes`。
- 建议只透传小的标量字段（env / id / temperature / 自定义 tag），避免透传 `gen_ai.input.messages` 等大字段。
...
| LLM | `gen_ai.usage.total_tokens` | SHOULD | 优先上游，缺失则算 input+output |
